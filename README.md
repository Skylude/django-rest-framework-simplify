 ...
 * POST `/sendEmail` `{'templateName': 'DynamicEmail', 'to': 'to_address@test.com', 'firstName': 'Chris', 'signUpUrl': 'https://github.com/Skylude/django-rest-framework-simplify', 'teamName': 'My Team'}` => This will create the email and call `EmailService.send_email` with your form data. You can then choose what to do with that.
 ...
 ### Settings
```python
REST_FRAMEWORK_SIMPLIFY_RAISE_INVALID_INCLUDES = False
```
If this setting is True any includes that are passed in the request will be validated against the model get_includes function. If not found a 400 Bad Request error will be thrown with the include field name in the error message.
