...
from tests.views import BasicClassHandler, ChildClassHandler, LinkingClassHandler, \
...
urlpatterns = [
    ...
    url(
        r'^(?P<parent_resource>[a-zA-Z]+)/(?P<parent_pk>[0-9]+)/childClassNoLinker$',
        LinkingClassWithNoLinkingClsDefinedHandler.as_view(),
    ),
    url(
        r'^(?P<parent_resource>[a-zA-z]+)/(?P<parent_pk>[0-9]+)/childClass/(?P<pk>[0-9]+)$',
