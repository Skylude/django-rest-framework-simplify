...
from rest_framework_simplify.helpers import parse_binary, parse_date
...
class SimplifyModel(DjangoModel):
        ...
        if hasattr(self, "change_tracking_fields"):
            for x in self._meta.local_concrete_fields:
                if x.name in self.change_tracking_fields:
                    setattr(
                        self,
                        f"_{x.attname}_initial",
                        self.__dict__.get(x.attname),
                    )

    def change_tracking_field_has_changed(self, field_name):
        if (
            hasattr(self, "change_tracking_fields")
            and field_name in self.change_tracking_fields
        ):
            field = self._meta.get_field(field_name)
            return getattr(self, f"_{field.attname}_initial") != getattr(
                self, field.attname,
            )
        else:
            raise Exception(
                f"{field_name} is not in the change_tracking_fields attribute",
            )
    ...
    def get_change_tracking_field_initial_value(self, field_name):
        if (
            ...
            field = self._meta.get_field(field_name)
            return getattr(self, f"_{field.attname}_initial")
        else:
            raise Exception(
                f"{field_name} is not in the change_tracking_fields attribute",
            )
    ...
    def parse(
        ...
        for field in fields:
            ...
            if field_name == "id":
                ...
                field_value = (
                    cc_field_value
                    if cc_field_value is not None
                    else data.get(field_name, None)
                )
                ...
                if type(field) is DjangoForeignKey or type(field) is OneToOneField:
                    ...
                    if (
                        ...
                        and field_name in reference_fields.keys()
                    ):
                        ...
                        try:
                            related_item = related_cls.objects.get(
                                pk=reference_fields[field_name],
                            )
                            setattr(obj, field_name, related_item)
                        # couldn't find id of related_item in db so not a valid model definition
                        except ObjectDoesNotExist:
                            raise ParseException(
                                ErrorMessages.RELATED_ITEM_DOES_NOT_EXIST.format(
                                    field_name,
                                ),
                            )
                    ...
                    # check if they are passing an id and want to simply change the foreign key id
                        ...
                        field.name + "_id" in data.keys()
                        or Mapper.underscore_to_camelcase(field.name + "_id")
                        in data.keys()
                    ):
                        # field_value will have _id in it
                        cc_field_name_id = Mapper.underscore_to_camelcase(
                            field_name + "_id",
                        )
                    ...
                    # check to see if we need to parse in another current_parse_level
                        ...
                        if current_parse_level <= cls.parseable_levels:
                            ...
                            if type(field_value) is dict:
                                ...
                                try:
                                    related_item = related_cls.parse(
                                        field_value,
                                        existing_id=related_item_id,
                                ...
                                # overall parsing failed if we didn't a nested item and we should have
                                    ...
                                    raise ParseException(
                                        ErrorMessages.PARSEABLE_RELATED_FIELD_PARSE_FAILED.format(
                                            ex.args[0],
                                        ),
                                    )
                                else:
                ...
                # if the field name was passed in try to update it
                    ...
                    if type(field) is DjangoDateTimeField and field_value:
                        ...
                        if not field_value:
                            raise ParseException(
                                ErrorMessages.COULD_NOT_PARSE_DATE_FIELD.format(
                                    field_name,
                                ),
                            )
                        ...
                        if not field_value:
                            raise ParseException(
                                ErrorMessages.COULD_NOT_PARSE_BINARY_FIELD.format(
                                    field_name,
                                ),
                            )
                    elif (
                        type(field) is SimplifyEncryptedCharField
                        or type(field) is SimplifyEncryptedField
                    ) and field_value:
                        obj.encrypted_fields.append(field_name)
                    elif type(field) is DecimalField and field_value:
                        decimal_places = field.decimal_places
                        field_value = round(
                            decimal.Decimal(field_value), decimal_places,
                        )
        ...
        try:
            obj.full_clean(
                exclude=["id"] + obj.related_items_to_be_saved + obj.encrypted_fields,
            )
