...
class SimplifyView(APIView):
    ...
    def delete(self, request, pk=None, parent_resource=None, parent_pk=None):
        if parent_pk and parent_resource and self.linked_objects:
            if "DELETE_SUB" not in self.supported_methods:
                return self.create_response(
                    error_message=ErrorMessages.DELETE_SUB_NOT_SUPPORTED.format(
                        self.model.__name__,
                    ),
                )
        else:
            if "DELETE" not in self.supported_methods:
                return self.create_response(
                    error_message=ErrorMessages.DELETE_NOT_SUPPORTED.format(
                        self.model.__name__,
                    ),
                )
        ...
        for linked_object in self.linked_objects:
            ...
            if linked_object["sub_resource_name"] and linked_object["linking_cls"]:
                kwargs = {linked_object["sub_resource_name"]: obj}
                # get the linking table items
                linked_objs = (
                    linked_object["linking_cls"]
                    .objects.using(self.read_db)
                    .filter(**kwargs)
                )
    ...
    def get(self, request, pk=None, parent_resource=None, parent_pk=None):
        ...
        if pk:
            ...
            if parent_resource and parent_pk and self.linked_objects:
                if "GET_SUB" not in self.supported_methods:
                    return self.create_response(
                        error_message=ErrorMessages.GET_SUB_NOT_SUPPORTED.format(
                            self.model.__name__,
                        ),
                    )
            ...
            if parent_pk and parent_resource and self.linked_objects:
                ...
                lives_on_parent_results = [
                    i
                    for i in self.linked_objects
                    if "lives_on_parent" in i
                    and i["lives_on_parent"]
                ...
                if len(lives_on_parent_results) > 0:
                    ...
                    child_id = (
                        linked_object["parent_cls"]
                        .objects.using(self.read_db)
                        .values(child_id_field_name)
                        .get(pk=parent_pk)[child_id_field_name]
                    )
                    ...
                    if "GET_LIST_SUB" not in self.supported_methods:
                        return self.create_response(
                            error_message=ErrorMessages.GET_LIST_SUB_NOT_SUPPORTED.format(
                                self.model.__name__,
                            ),
                        )
                ...
                if "GET_LIST" not in self.supported_methods:
                    return self.create_response(
                        error_message=ErrorMessages.GET_LIST_NOT_SUPPORTED.format(
                            self.model.__name__,
                        ),
                    )
        ...
        model_includes = (
            self.model.get_includes() if hasattr(self.model, "get_includes") else []
        )
        include = [
            Mapper.camelcase_to_underscore(include.strip())
            for include in req_includes
            if Mapper.camelcase_to_underscore(include.strip()) in model_includes
        ]
                ...
                for include_field_tree in includes_on_model:
                    for include_field in include_field_tree[1:]:
                        if hasattr(include_field, "related_model"):
                            exclude_fields = (
                                include_field.related_model.get_excludes()
                                if hasattr(include_field.related_model, "get_excludes")
                                else []
                            )
                            include_fields = [
                        ...
                        if (
                            hasattr(include_field, "multiple")
                            and include_field.multiple
                        ):
                            multi_field.append(include_field.name)
            else:
                for include_field_tree in includes_on_model:
                    if (
                        len(include_field_tree) == 2
                        and type(include_field_tree[1]) == ForeignKey
                    ):
                        obj = obj.select_related(include_field_tree[0])
                    elif type(include_field_tree[1]) in (
                        ForeignKey,
                        ManyToManyRel,
                        ManyToOneRel,
                        OneToOneRel,
        ...
        if filters:
            ...
            for filter in filters:
                ...
                if hasattr(self.model, "get_filterable_properties"):
                    filterable_property = (
                        filter_name in self.model.get_filterable_properties().keys()
                    )
                else:
                ...
                if filter_name in model_filters.keys():
                    if model_filters[filter_name]["list"]:
                        filter_value = [
                            self.format_filter(filter_name, item, model_filters)
                        ...
                        if exclude_filter:
                            ...
                            filterable_property_query = (
                                self.model.get_filterable_properties()[filter_name][
                                    "query"
                                ]
                            )
                            ...
                            filterable_properties[
                                filter_name.rstrip("__in")
                            ] = filterable_property_query
                            filterable_property_kwargs[filter_name] = filter_value
                        else:
            ...
            if filterable_properties:
                obj = (
                    obj.using(self.read_db)
                    .annotate(**filterable_properties)
            ...
            for filter_name, filter_value in isolated_filter_kwargs.items():
                ...
                for x in range(0, len(filter_value)):
                    kwargs = {f"{filter_name}": filter_value[x]}
        ...
        # setup excludes
        excludes = (
            self.model.get_excludes() if hasattr(self.model, "get_excludes") else []
        )
        ...
        if simple:
            ...
            for primary_key in body_by_primary_key:
                ...
                if full_includes:
                    for body_item in body_items:
                        for include_field in full_includes:
                            field_names = [
                                field
                                for field in body_item
                                if include_field + "__" in field
                            ]
                            field_names_to_remove = [
                                field for field in field_names if field not in include
                            ]

                            body_item[include_field] = {
                                field_name.replace(include_field + "__", ""): body_item[
                                    field_name
                                ]
                            ...
                            if all(
                                val == None for val in body_item[include_field].values()
                            ):
                ...
                if len(body_items) > 1:
                    ...
                    if len(differences) > 0:
                        ...
                        for difference in differences:
                            all_items = [
                                body_item[difference] for body_item in body_items
                            ]
                            if all(type(item) is dict for item in all_items):
                                ...
                                item[difference] = [
                                    dict(tupleized)
                                    for tupleized in {
                                        tuple(item.items()) for item in all_items
                                    }
                                ]
                            else:
                ...
                for field_name in multi_field:
                    if (
                        not type(body_by_primary_key[primary_key][0][field_name])
                        is list
                    ):
            ...
            if linked_object["parent_resource"] == parent_resource:
                ...
                if linked_object["parent_cls"]:
                    if not linked_object["linking_cls"]:
                        ...
                        if hasattr(field, "multiple") and field.multiple:
                            kwargs = {parent_field_name + "__id": parent_pk}
                        else:
                            kwargs = {parent_field_name + "_id": parent_pk}
                    else:
                        kwargs = {linked_object["parent_name"] + "_id": parent_pk}
                else:
                    kwargs = {linked_object["parent_name"]: parent_pk}
    ...
    def post(self, request, parent_resource=None, parent_pk=None):
        ...
        if (
            parent_resource
            and parent_pk
            and self.model.resource_mapping
            and parent_resource in self.model.resource_mapping.keys()
        ):
            reference_fields = {self.model.resource_mapping[parent_resource]: parent_pk}
        ...
        if id and not hasattr(self, "linked_objects"):
            return self.create_response(
                error_message=ErrorMessages.POST_SUB_WITH_ID_AND_NO_LINKING_CLASS.format(
                    self.model.__name__,
                ),
            )
        ...
        if parent_pk and parent_resource and self.linked_objects:
            ...
            self.execute_on_linked_object(
                obj,
                self.linked_objects,
                parent_resource,
                parent_pk,
                snake_cased_url_tail,
                self.write_db,
            )
        ...
        if links:
            for link in links.split("|"):
                ...
                link_model = obj.__class__._meta.get_field(
                    Mapper.camelcase_to_underscore(model),
                ).related_model
                back_reference_field_name = next(
                    x
                    for x in link_model._meta.get_fields()
                    if hasattr(x, "related_model") and x.related_model == obj.__class__
                ).attname
                link_model(
                    **{
                        back_reference_field_name: obj.id,
                        Mapper.camelcase_to_underscore(link_id_field_name): link_id,
                    },
                ).save()
    ...
    def put(self, request, pk):
        if "PUT" not in self.supported_methods:
            return self.create_response(
                error_message=ErrorMessages.PUT_NOT_SUPPORTED.format(
                    self.model.__name__,
                ),
            )
        ...
        if serialize:
            if body is None:
                ...
                if count is not None:
                    body = {"count": count if count != -1 else None, "data": body}
    ...
    @staticmethod
    def execute_on_linked_object(
        obj,
        linked_objects,
        passed_in_parent_resource,
        parent_pk,
        snake_cased_url_tail,
        write_db,
    ):
        linking_class_results = [
            i
            for i in linked_objects
            if i["parent_resource"] == passed_in_parent_resource and i["linking_cls"]
        ]
        lives_on_parent_results = [
            i
            for i in linked_objects
...
class SimplifyStoredProcedureView(APIView):
    ...
    def post(self, request):
        ...
        if hasattr(self.forms, form_name):
            ...
            return Response(
                {
                    "errorMessage": self.ErrorMessages.INVALID_STORED_PROCEDURE.format(
                        sp_name,
                    ),
                },
        ...
        if form.is_valid():
            ...
            # not valid sp params
            return Response(
                {"errorMessage": self.ErrorMessages.INVALID_PARAMS.format(sp_name)},
                status=status.HTTP_400_BAD_REQUEST,
            )
...
class SimplifyEmailTemplateView(APIView):
    ...
    class ErrorMessages:
        ...
        UNABLE_TO_POPULATE_TEMPLATE = (
            "Unable to populate all the needed fields in {0}. Field: {1}"
        )
    ...
    def post(self, request):
        ...
        if hasattr(self.templates, form_name):
            ...
            return Response(
                {
                    "errorMessage": self.ErrorMessages.INVALID_EMAIL_TEMPLATE.format(
                        template_name,
                    ),
                },
                status=status.HTTP_400_BAD_REQUEST,
