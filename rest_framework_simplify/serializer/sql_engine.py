...
class SQLEngineSerializer:
    ...
    def model_to_dict(self, manager_query_or_model):
        ...
        raw_models = [self.one_model_to_dict(model) for model in manager_query_or_model]
    ...
    def one_model_to_dict(self, model):
        (
            ...
            binary_fields,
            all_fields,
        ) = self.get_fields_by_type(type(model))
        ...
        if self.fields:
            field_names = [
                ...
                or Mapper.string_underscore_to_camelcase("{}_id".format(field_name))
                in self.fields
            ]
        else:
    ...
    def get_fields_by_type(self, model_type):
        if model_type not in memoized_type_mappings:
            ...
            decimal_fields = set(
                (field.name for field in all_field_types if type(field) is DecimalField)
            )
            binary_fields = set(
                (field.name for field in all_field_types if type(field) is BinaryField)
            )
            memoized_type_mappings[model_type] = (
                foreign_key_fields,
                decimal_fields,
                binary_fields,
                all_fields,
        ...
        if type(view_model) is list:
            for idx, vm in enumerate(view_model):
                if hasattr(field_obj[idx], "get_excludes"):
                    self.remove_excludes_from_view_model(
                        field_obj[idx].get_excludes(), vm
                    )

        else:
            if hasattr(field_obj, "get_excludes"):
                self.remove_excludes_from_view_model(
                    field_obj.get_excludes(), view_model
                )
