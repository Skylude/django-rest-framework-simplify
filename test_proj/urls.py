...
from test_app.views import BasicClassHandler, ChildClassHandler, LinkingClassHandler, \
    ...
    OneToOneHandler, RequestFieldSaveHandler, PhaseGroupHandler

urlpatterns = [
    url(r'^(?P<parent_resource>[a-zA-z]+)/(?P<parent_pk>[0-9]+)/childOne$', ChildClassHandler.as_view()),
    url(r'^(?P<parent_resource>[a-zA-Z]+)/(?P<parent_pk>[0-9]+)/childClassNoLinker$', LinkingClassWithNoLinkingClsDefinedHandler.as_view()),
    url(r'^(?P<parent_resource>[a-zA-z]+)/(?P<parent_pk>[0-9]+)/childClass/(?P<pk>[0-9]+)$', LinkingClassHandler.as_view()),
